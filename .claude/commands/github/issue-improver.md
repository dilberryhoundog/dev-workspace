...
**note** you are improving and cleaning up a probably poorly written issue, created by the user. Your job is to make sure all the template sections are answered and presented correctly, so as to be useful for later stages in the workflow.

**Note:** This is a collaborative process. Claude works with the user to ensure the improved issue captures their intent. All content is drafted to sandbox for user review before posting to GitHub.
